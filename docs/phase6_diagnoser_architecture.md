...
3. **`AnswerRelevanceRule`**
  ...
  - **Actionable Advice:** Enable query transformations (`query_rewrite` / `step_back`) and inject a capable reranker (e.g., `cross_encoder_miniLM` or `llm_rerank`) to aggressively filter out middle-ranked noise.
