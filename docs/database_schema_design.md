...
## Table 2.5: `evaluation_job_history` (Deprecated -> Replaced by `evaluation_results` Model)
*(Archived: Replaced by the wide-table architecture in Table 11 `evaluation_results` which tracks `evaluation_strategy` and `judge_model` directly per query without needing a separate job entity)*

---

## Table 3: `evaluation_metrics` (Deprecated EAV Model -> Replaced by `evaluation_results`)
*(Archived: Replaced by the wide-table architecture in Table 11 `evaluation_results`. The former Entity-Attribute-Value (EAV) model lacked the ability to efficiently store large TEXT blocks like the LLM Judge's `reasoning`, and required complex PIVOT queries for basic dashboards.)*
