...
*   **`PgVectorRetrieverDAO`**: Implements `IRetrieverDAO` using `psycopg` and `pgvector`. 
    ...
    *   **Architecture Rationale: Imperative Orchestration vs. Pure Declarative LCEL in RAGAgent**:
        ...
        1. **Lines of Code**: Pure LCEL is highly concise, replacing multi-line conditional logic with a single pipeline declaration. However, in our context, the imperative approach requires only a few extra lines to package the `RAGResponse`.
    ...
    *   **Architecture Rationale: Native `psycopg3` vs SQLAlchemy ORM/Core**:
        ...
        2. **Higher Performance Ceiling**: The core logic relies on highly PostgreSQL-specific syntax (e.g., `<=>` for Cosine Distance, `ANY(%s)` array matching). SQLAlchemy's primary value proposition—database-agnostic SQL translation and object state tracking (Identity Map)—becomes pure overhead here. By compiling raw, optimized SQL strings, we eliminate the Abstract Syntax Tree (AST) translation tax, drastically reducing CPU cycles and memory footprint during high-throughput RAG evaluation sweeps.
